...
CUBLASAPI cublasStatus_t CUBLASWINAPI cublasCreate_v2(cublasHandle_t *handle) {
  ...
  std::cout << get_global_ozimmu_handle()->cublas_handle << std::endl;
  // printf("calling cublasCreate_v2->cublasCreate_org\n");
...
CUBLASAPI cublasStatus_t cublasGemmEx(
  ...
  // printf("cublasGemmEx==%d ",compute_mode == mtk::ozimmu::dgemm);
  if (compute_mode != mtk::ozimmu::dgemm &&
    ...
    if (compute_mode == mtk::ozimmu::sgemm) {
      ...
      // mtk::ozimmu::enable_profiling(get_global_ozimmu_handle());
    ...
    // mtk::ozimmu::print_profiler_result(get_global_ozimmu_handle(),
    //                                     "", false);
...
CUBLASAPI cublasStatus_t CUBLASWINAPI cublasDgemm_v2(
  ...
  // printf("ozdgemm m = %d, n = %d, k = %d, lda = %d, ldb = %d, ldc = %d\n", m, n, k,lda,ldb,ldc);
  // printf("ozdgemm ");
